...
public class Alumno {
	...
	public void leerAlumno(RandomAccessFile raf) {
		StringBuffer sb;
		try {
			raf.seek((num_alumno-1)*TAM_REGISTRO);
			System.out.println("Numero: "+ raf.readInt());
			System.out.println("Nombre: " + this.leerString(TAM_NOMBRE, raf));
			System.out.println("Domicilio: " + this.leerString(TAM_DOMICILIO, raf));
			System.out.println("Edad: "+ raf.readInt());
			System.out.println("Telefono: " + raf.readInt());
			System.out.println("Correo: " + this.leerString(TAM_CORREO, raf));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void aniadirString(int tamanio,String cadena, RandomAccessFile raf) {
	...
	private String leerString(int tamanio, RandomAccessFile raf) {
		StringBuffer sb = new StringBuffer(tamanio);
